...
def get_config_permissions(config_id):
    ...
    try:
        return configs_permissions_store[config_id]
    except KeyError:
...
def set_config_permission(user, config_id, level):
    ...
    if not isinstance(level, PermissionLevel):
        raise errors.DAOError("level must be a PermissionLevel object.")
    new = configs_permissions_store.add_if_empty([config_id, user], str(level))
    if not new:
        configs_permissions_store[config_id, user] = str(level)
    logger.info(f"Permission set for config: {config_id}; user: {user} at level: {level}")
