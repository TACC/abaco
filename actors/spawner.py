...
class Spawner(object):
    ...
    def process(self, cmd):
        ...
        try:
            ...
            i='actor'               #i identify the type of process we got
        except:
        ...
        try:
            ...
            msg = f"Exception in spawner trying to retrieve object from store. Aborting. Exception: {e}"
            logger.error(msg)
            return
        if i=='actor':
        ...
        if i=='actor':
            ...
            AdapterServer.update_status(adapter_id, server_id, SPAWNER_SETUP)
    ...
    def start_worker(self,
        ...
        while True:
            try:
                ...
                logger.error(f"Spawner got a docker exception from run_worker; Exception: {e}")
                if 'read timeout' in e.message:
                    logger.info("Exception was a read timeout; trying run_worker again..")
                    time.sleep(5)
                    attempts = attempts + 1
                    if attempts > 20:
                        msg = f"Spawner continued to get DockerError for 20 attempts. Exception: {e}"
                        logger.critical(msg)
