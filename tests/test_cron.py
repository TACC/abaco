...
def test_register_actor_with_cron(headers):
    ...
    data = {'image': 'jstubbs/abaco_test', 'name': 'cron_test', 'cronSchedule': '2099-06-21 14 + 6 hours'} 
    rsp = requests.post(url, data=data, headers=headers)
    result = basic_response_checks(rsp)
    assert result['cronSchedule'] == '2099-06-21 14 + 6 hours'

def test_register_actor_with_incorrect_cron(headers):
    url = '{}/{}'.format(base_url, 'actors')
    data = {'image': 'jstubbs/abaco_test', 'name': 'cron_test', 'cronSchedule': '2099-06-21 14 + 6 flimflams'} 
    rsp = requests.post(url, data=data, headers=headers)
    #result = basic_response_checks(rsp)
    assert rsp.status_code == 500

def test_update_cron(headers):
    actor_id = get_actor_id(headers, name='cron_test')
    url = '{}/actors/{}'.format(base_url, actor_id)
    data = {'image': 'jstubbs/abaco_test', 'stateless': False, 'cronSchedule': '2099-09-6 20 + 3 months'}
    rsp = requests.put(url, data=data, headers=headers)
    result = basic_response_checks(rsp)
    assert result['cronSchedule'] == '2099-09-6 20 + 3 months'

def test_update_cron_switch(headers):
    actor_id = get_actor_id(headers, name='cron_test')
    url = '{}/actors/{}'.format(base_url, actor_id)
    data = {'image': 'jstubbs/abaco_test', 'stateless': False, 'cronSchedule': '2099-09-6 20 + 3 months', 'cronOn': False}
    rsp = requests.put(url, data=data, headers=headers)
    result = basic_response_checks(rsp)
