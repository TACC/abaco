...
if [ $api = "reg" ]; then
    ...
    if [ $server = "dev" ]; then
        ...
        cd /home/tapis/actors; /usr/local/bin/gunicorn --workers $processes --threads $threads --timeout $timeout -b :5000 reg_api:app
    fi
elif [ $api = "admin" ]; then
    if [ $server = "dev" ]; then
        python3 -u /home/tapis/actors/admin_api.py
    else
        cd /home/tapis/actors; /usr/local/bin/gunicorn --workers $processes --threads $threads --timeout $timeout -b :5000 admin_api:app
    fi
elif [ $api = "metrics" ]; then
    if [ $server = "dev" ]; then
        python3 -u /home/tapis/actors/metrics_api.py
    else
        cd /home/tapis/actors; /usr/local/bin/gunicorn --workers $processes --threads $threads --timeout $timeout -b :5000 metrics_api:app
    fi
elif [ $api = "mes" ]; then
    if [ $server = "dev" ]; then
